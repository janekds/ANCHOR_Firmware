...
// ── Palette ───────────────────────────────────────────────────
#define C_BG      0x0A0F1A   // deep navy — screen background
#define C_CARD    0x0E1F33   // card surface
#define C_HDR     0x0D47A1   // header blue
#define C_ACCENT  0x29B6F6   // sky blue accent
#define C_CYAN    0x00E5FF
#define C_BORDER  0x1A3A5C
#define C_GREEN   0x00C853
#define C_ORANGE  0xFF6D00
#define C_WHITE   0xFFFFFF
#define C_GREY    0x90A4AE
#define C_DGREY   0x37474F
...
static void create_welcome_screen() {
    ...
    // ── Brightness slider card ──
    {
        ...
        add_label(rh, LV_SYMBOL_IMAGE "  Brightness", C_GREY);
        lbl_brightness_val = add_label(rh, "75%", C_ACCENT);
    ...
    // ── Flow rate slider card ──
    {
        ...
        lv_obj_t *rh = make_row(card, C_CARD);
        add_label(rh, LV_SYMBOL_LOOP "  Flow Rate", C_GREY);
        lbl_flow_val = add_label(rh, "200 GPH", C_CYAN);

        make_slider(card, C_BORDER, C_CYAN, 0, 500, 200, slider_flow_cb);
    }
    ...
    // ── Sensor readout card ──
    {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            add_label(rw, rows[i], C_GREY, &lv_font_montserrat_14);
        }
    }
    ...
    lv_obj_t *ls = add_label(btn_skip, "Skip", C_GREY, &lv_font_montserrat_14);
    lv_obj_center(ls);
...
static void create_wifi_screen() {
    ...
    add_label(net_row, "Network:", C_GREY, &lv_font_montserrat_10);
    lbl_wifi_network = add_label(net_row, "—", C_ACCENT, &lv_font_montserrat_10);

    lv_obj_t *ip_row = make_row(status_card, C_CARD);
    add_label(ip_row, "Setup page:", C_GREY, &lv_font_montserrat_10);
    lbl_wifi_ip = add_label(ip_row, "—", C_CYAN, &lv_font_montserrat_10);

    // ── Instructions ──
    lv_obj_t *sec = make_card(pane, C_BG, C_BG, 0, 0);
    lv_obj_set_style_pad_all(sec, 2, 0);
    add_label(sec, "SETUP STEPS", 0x4A6FA5, &lv_font_montserrat_10);
    ...
    for (auto &s : steps) {
        ...
        lv_obj_t *lbl = add_label(card, s.txt, C_WHITE, &lv_font_montserrat_14);
